...
⚠️ 判据是「这个标记已经被启动过几次」，不是「标记在不在」：
装完包标记留在盘上是**正常**的（那次 `/restart` 的启动就是来验证它的），所以
**第一次**带标记启动必须放行；只有**第二次**还带着它启动，才说明上次启动没走到验证
那一步 → 才回滚。老实现只看「标记在不在」就回滚，导致
「`/update confirm` → `/restart`」这条**正常流程 100% 被自己回滚掉**
（v1.0.6 线上实测：回滚发生在 `Waiting for application startup` 之前，新版本连一次
启动机会都没有）。`/update rollback` 仍可手动回滚，`/status` 会显示待验证状态。
